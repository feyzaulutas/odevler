...
# 5 öneri user-based modelden 5 öneri de item-based modelden ele alınız ve nihai olarak
# 10 öneriyi 2 modelden yapınız.
import pandas as pd
import math
...
# Adım 2: Herbir kullancının seçili user'in izlediği filmlerin kaçını izlediği bilgisini taşıyan
...
final_df.head(20)
# Adım 2: Kullanıcıların birbirleri ile olan korelasyonlarının bulunacağı
# yeni bir corr_df dataframe’i oluşturunuz.
...
# Adım 3 :User based recommendation bölümünde oluşturulan user_movie_df dataframe’ini seçilen
# film id’sine göre filtreleyiniz.
# user_movie_df = common_movies.pivot_table(index=["userId"], columns=["title"], values="rating")
...
movies_from_item_based = item_based_recommender(movie[movie["movieId"] == movie_id][
                                                    "title"].values[0], user_movie_df)

movies_from_item_based[1:6].index.tolist()
