...
df.drop("SepetID", axis=1, inplace=True)
...
sepet_hizmet_df = df.pivot_table(index='SepetID', columns='Hizmet', aggfunc='size', fill_value=0).applymap(lambda x: 1 if x > 0 else 0)
sepet_hizmet_df.iloc[0:5, 0:20]
...
def create_rules(dataframe):
    dataframe = df.pivot_table(index='SepetID', columns='Hizmet', aggfunc='size', fill_value=0).applymap(lambda x: 1 if x > 0 else 0)
...
# 2_0 hizmeti alan kullanıcıya 3 öneri = ['15_1', '22_0', '25_0']
